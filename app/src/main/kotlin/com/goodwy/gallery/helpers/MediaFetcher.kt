...
class MediaFetcher(val context: Context) {
    ...
    fun getFilesFrom(
        ...
        allSizes: HashMap<String, Long>? = null, allVideoDurations: HashMap<String, Int>? = null
    ): ArrayList<Medium> {
    ...
    private fun getMediaInFolder(
        ...
        // Use pre-loaded batch maps if available, else fall back to per-folder queries
        val fileSizes: HashMap<String, Long> = if (checkProperFileSize || checkFileExistence) {
            allSizes ?: getFolderSizes(folder)
        } else HashMap()
        val videoDurationsCache: HashMap<String, Int> = if (getVideoDurations) {
            allVideoDurations ?: getFolderVideoDurations(folder)
        } else HashMap()
        ...
        for (curFile in files) {
            ...
            if (isRecycleBin) {
                ...
                val videoDuration = if (getVideoDurations && isVideo) {
                    val cached = videoDurationsCache[path] ?: 0
                    // Se não veio do cache, tenta via MediaMetadataRetriever
                    if (cached == 0) getDurationViaRetriever(path) else cached
                } else 0
        ...
        context.queryCursor(uri, projection) { cursor ->
            ...
            try {
                ...
                val videoDuration = if (isVideo) {
                    (cursor.getLongValue(MediaStore.MediaColumns.DURATION) / 1000.0).roundToInt()
                } else 0
                val isFavorite = favoritePaths.contains(path)
                val medium =
    ...
    // Single global query for all file sizes — replaces per-folder getFolderSizes() calls
    fun getAllSizes(): HashMap<String, Long> {
    ...
    // Single global query for all video durations — replaces per-folder getFolderVideoDurations() calls
    fun getAllVideoDurations(): HashMap<String, Int> {
        val durations = HashMap<String, Int>()

        // Usa Video.Media URI — mais confiável para DURATION do que Files.getContentUri
        val projection = arrayOf(
        ...
        try {
            context.queryCursor(uri, projection) { cursor ->
                try {
                    val durationMs = cursor.getLongValue(MediaStore.Video.Media.DURATION)
                    val path = cursor.getStringValue(MediaStore.Video.Media.DATA)
                    if (durationMs > 0 && path.isNotEmpty()) {
                        durations[path] = (durationMs / 1000.0).roundToInt()
                    }
                } catch (_: Exception) {}
            }
        } catch (_: Exception) {}
    ...
    private fun getDurationViaRetriever(path: String): Int {
        return try {
            ...
            // Persiste no BD para não recalcular na próxima vez
            if (durationSec > 0) {
    ...
    private fun getFolderVideoDurations(folder: String): HashMap<String, Int> {
        val durations = HashMap<String, Int>()
        if (folder == FAVORITES || folder == RECYCLE_BIN) return durations
