...
class GetMediaAsynctask(
    ...
    override fun doInBackground(vararg params: Void): ArrayList<ThumbnailItem> {
        ...
        // Single batch query for video durations — avoids one MediaStore query per folder
        val allVideoDurations = if (getVideoDurations) mediaFetcher.getAllVideoDurations() else null
        val allSizes = if (getProperFileSize) mediaFetcher.getAllSizes() else null
        ...
        val media = if (showAll) {
            ...
            foldersToScan.forEach {
                val newMedia = mediaFetcher.getFilesFrom(
                    ...
                    allSizes = allSizes, allVideoDurations = allVideoDurations
                )
