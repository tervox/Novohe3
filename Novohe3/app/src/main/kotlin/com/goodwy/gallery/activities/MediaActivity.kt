...
class MediaActivity : SimpleActivity(), MediaOperationsListener {
    ...
    private var mFolderSizeStr = ""   // tamanho calculado, ex: "17,3 GB"
...
//            getMedia()
    ...
    private fun tryLoadGallery() {
        requestMediaPermissions {
            ...
            val searchHint = if (mShowAll) {
                ...
                getString(com.goodwy.commons.R.string.search_in_placeholder, mDirName)
            }
            ...
            // Mantém o tamanho no título se já foi calculado antes
            val cachedSize = if (!mShowAll && config.showFolderSize) config.getCachedFolderSize(mPath) else ""
            if (cachedSize.isNotEmpty()) mFolderSizeStr = cachedSize

            val titleWithSize = if (!mShowAll && config.showFolderSize && mFolderSizeStr.isNotEmpty()) {
                "$mDirName • $mFolderSizeStr"
            } else if (mShowAll) {
                resources.getString(com.goodwy.strings.R.string.library)
            } else {
                mDirName
            }
            binding.mediaMenu.updateTitle(titleWithSize)
            getMedia()
            setupLayoutManager()
        }
    }
    ...
    private fun setupTabsHide() {
        ...
        binding.mediaGrid.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            ...
            // Pausa Glide durante scroll rápido — elimina travamento com GIFs e vídeos
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                super.onScrollStateChanged(recyclerView, newState)
                when (newState) {
                    RecyclerView.SCROLL_STATE_DRAGGING,
                    RecyclerView.SCROLL_STATE_SETTLING -> Glide.with(this@MediaActivity).pauseRequests()
                    RecyclerView.SCROLL_STATE_IDLE -> Glide.with(this@MediaActivity).resumeRequests()
                }
            }
        })
    }
    ...
    private fun gotMedia(media: ArrayList<ThumbnailItem>, isFromCache: Boolean) {
        ...
        runOnUiThread {
            ...
            // Mostra tamanho total da pasta no título (ex: "Vídeos • 1,2 GB")
            if (!mShowAll && config.showFolderSize && media.isNotEmpty()) {
                val totalSize = media.filterIsInstance<Medium>().sumOf { it.size }
                if (totalSize > 0) {
                    mFolderSizeStr = totalSize.formatSize()
                    config.setCachedFolderSize(mPath, mFolderSizeStr)   // persiste entre sessões
                    binding.mediaMenu.updateTitle("$mDirName • $mFolderSizeStr")
                }
            }
        }
        ...
        if (!isFromCache) {
            ...
            Thread {
                try {
                    // 1. Carrega todas as durações salvas no BD (UMA query só)
                    val savedDurations: Map<String, Int> = try {
                        mediaDB.getVideoDurationsMap().associate { it.path to it.duration }
                    } catch (_: Exception) { emptyMap() }

                    // 2. Restaura duração calculada nos itens que o MediaStore retornou como 0
                    //    Isso evita que o REPLACE apague o valor que calculamos antes
                    ...
                    // 3. Insere/atualiza no BD (duração calculada já está preservada)
                    mediaDB.insertAll(mediaToInsert)
                } catch (_: Exception) {}

                // 4. Calcula via MMR apenas os vídeos que AINDA ficaram com duração 0
                if (config.showThumbnailVideoDuration) {
                    val videosStillMissing = mediaToInsert.filter { it.isVideo() && it.videoDuration == 0 }
                    videosStillMissing.forEach { medium ->
                        try {
                            ...
                            if (durationSec > 0) {
                                ...
                                mediaDB.updateVideoDuration(medium.path, durationSec)
                                runOnUiThread {
