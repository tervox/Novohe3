...
fun Context.loadImageBase(
    ...
    val isAnimatedGif = animate && roundCorners == ROUNDED_CORNERS_NONE && (path.isGif() || path.isWebP())
    val options = RequestOptions()
        .signature(signature)
        .skipMemoryCache(skipMemoryCacheAtPaths?.contains(path) == true)
        .priority(if (isAnimatedGif) Priority.NORMAL else Priority.LOW)
        .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
        .format(if (isAnimatedGif) DecodeFormat.PREFER_RGB_565 else DecodeFormat.PREFER_ARGB_8888)
        // GIFs em miniatura: 192px economiza ~44% de memória vs 256px sem perda visual perceptível
        .override(if (isAnimatedGif) 192 else 512, if (isAnimatedGif) 192 else 512)
    ...
    // Para GIFs animados: carrega frame estático rápido antes do GIF completo
    // Usa asDrawable() + dontAnimate() para compatibilidade de tipo com o builder principal
    if (isAnimatedGif && path.isGif()) {
        val staticThumb = Glide.with(applicationContext)
            .asDrawable()
            .load(path)
            .apply(
                RequestOptions()
        ...
        builder = builder.thumbnail(staticThumb)
    }
